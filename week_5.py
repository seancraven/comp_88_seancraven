...
def init_layer(fan_in, fan_out, act, rng):
    ...
    he = (6 / fan_in) ** 0.5
    W = rng.uniform(low=-he, high=he, size=(fan_in, fan_out))
    b = np.zeros(fan_out)
    layer = {
        "W": W,
        "b": b,
        "shape": f"layer with {fan_in*fan_out +  b} free parameters",
        "act": act,
    }
    return layer
...
def init_mlp(spec, rng):
    ...
    assert len(spec) > 1
    mlp = []
    for (fan_in, act_1), (fan_out, _) in zip(spec, spec[1:]):
        mlp.append(init_layer(fan_in, fan_out, act_1, rng))
    return mlp
...
def layer_forward(layer, X):
    ...
    act_dict = {
        "sigmoid": sigmoid,
        "relu": relu,
        "binary cross entropy": binary_crossentropy_loss,
    }
    layer["X"] = X
    layer["Z"] = X @ layer["W"]
    layer["A"] = act_dict[layer["act"]](layer["Z"])
    return layer["A"]
...
def layer_backward(layer, dA):
    ...
    derivatives = {
        "sigmoid": d_sigmoid,
        "relu": d_relu,
        "binary cross entropy": d_binary_crossentropy_loss,
    }
    dZ = dA * derivatives[layer["act"]](layer["Z"])
    dX = dZ @ layer["W"].T
    dW = layer["X"].T @ dZ
    layer["db"] = np.sum(dZ, axis=1)
    layer["dA"], layer["dZ"], layer["dW"], layer["dX"] = dA, dZ, dW, dX
    return dX
...
def mlp_epoch(mlp, X, y, batch, lr, rng):
    ...
    """
    l_list = []
    for X_b, y_b in batch_gen(X, y, rng, batch):
        l_list.append(mlp_minibatch(mlp, X_b, y_b, lr))
    return np.mean(l_list)


def batch_gen(X, y, rng, batch):
    rand = np.random.default_rng(rng)
    ind = np.arange(len(y))
    rand.shuffle(ind)
    X_s, y_s = X[ind], y[ind]
    for i in range(0, len(y), batch):
        yield X_s[i : i + batch], y_s[i : i + batch]
...
def mlp_predict(mlp, X, thresh=0.5):
    ...
    return np.array([mlp_forward(mlp, X) > thresh]) * 1
...
def process_args():
    ...
    ap.add_argument(
        "-e", "--epochs", help="number of epochs to train", default=100, type=int
    )
    ap.add_argument(
        "-b", "--batch", help="minibatch size to train", default=1, type=int
    )
    ap.add_argument(
        "-y", "--layers", help="comma-sep list of hidden layer sizes", default="4,3"
    )
    ap.add_argument(
        "-d",
        "--data",
    ...
    ap.add_argument(
        "file", help="name of output file to produce", nargs="?", default="week_5.pdf"
    )
...
if __name__ == "__main__":
    ...
    layers = (
        ((2, "relu"),)
        + tuple((int(x), "relu") for x in hidden[:-1])
        + ((hidden[-1], "sigmoid"), (1, None))
    )
    ...
    if (aa is None) and (dd is None):
        ...
        if dd is not None:
            axs[0, 0].plot(xx, dd, color=orange, linestyle="dashed", label="Gradient")
    ...
    if (aa is None) and (dd is None):
        ...
        if dd is not None:
            axs[0, 1].plot(xx, dd, color=orange, linestyle="dashed", label="Gradient")
    ...
    if (ll0 is None) and (ll1 is None) and (dd0 is None) and (dd1 is None):
        ...
        if dd0 is not None:
            axs[0, 2].plot(
                yy, dd0, color=blue, linestyle="dashed", label="Gradient, $y=0$"
            )
        if ll1 is not None:
            axs[0, 2].plot(yy, ll1, color=orange, label="Loss, $y=1$")
        if dd1 is not None:
            axs[0, 2].plot(
                yy, dd1, color=orange, linestyle="dashed", label="Gradient, $y=1$"
            )
